...
In general, a QRF is a physical system associated with a \textit{symmetry group} $G$, under which it transforms covariantly. The properties of other systems can then be described relative to the chosen QRF with so-called \textit{relational observables}. The descriptions relative to different QRFs are related by \textit{QRF transformations}.
